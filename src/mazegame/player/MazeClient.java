...
public class MazeClient {
	...
	public static void createClient(String host, int port, MNodeType type) {
		logger.info("[START_NEW_CLIENT] type: {}...", type);
		MazeClientHandler.instance().setNodeType(type);
		try {
			connect(host, port);
		} catch (Exception ex) {
			System.out.println("Fail to connect primary server...");
	...
	/**
	 * Set up the connection to maze server. This method is called when player
	 * is started or master is crashed.
	 * 
	 * @param host
	 * @param port
